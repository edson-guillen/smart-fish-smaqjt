...
### Descrição Geral
O projeto de Programação Orientada a Objetos (POO) consiste em um conjunto de classes Java que desempenham funções específicas em um sistema. A seguir, são apresentados detalhes sobre as classes, atributos, métodos e práticas utilizadas no projeto.
...
- No arquivo SensorDto.java, a classe SensorDto possui atributos imutáveis, como `identifier

`, `temp`, `ph`, `water_level`, `turbidity` e `created_at`, uma vez que são declarados como `final`.
